...
6. 条目有体积预算（本文件已 60 KB+，读一次约 18k tokens）：每条 ≤ 6 行，只写「描述 / 结果 / 门禁」，过程细节留在代码注释与提交信息里；已完成批次压成一行归档。
...
- [x] **T28 高亮观感：激活/待切换区分度不足 + 选区着色发浑**
  - 现象（用户两张截图，都是浅色主题）：① `#fef08a`（激活）与 `#fef9c3`（待切换）是同色系深浅，肉眼几乎分不出哪段是当前项（「区分不够明显」）；② 选中文字时高亮整块变成橄榄泥色（「选文字的颜色也很难看」）。
  - 取证（对截图逐像素取样，不靠肉眼猜）：未选中态量到 `#fef08a` / `#fef9c3`；选中态量到 `#b8af67` = `#fef08a` × 0.72 —— 正是「30% 近黑选区罩在荧光黄上」的合成结果（同图里白底也被压成 `#b9b9b9`，同一个 30%）。所以两个问题都是**配色**问题，与渲染/对齐无关。
  - 修法（按产品「最小改动」定稿）：
    1. 待切换项改成**极浅中性灰 `#f3f4f6`**（浅色主题）+ 深色字：与激活项的荧光黄靠**饱和度**拉开（0.14 vs 0.98），不再走同色系深浅。深色主题仍用 `hsl(240 5% 28%)` —— `#f3f4f6` 在深色卡片上是近乎白的亮块，既刺眼又不再是「次级」。
    2. **选区配色不自定义**：删掉 `.tw-area-input::selection` 与 `.json-editor__input::selection` 两条规则（含上一步刚加的 `--tk-selection` 令牌），选区底色与选区文字色一律交回浏览器默认 —— 自定义的选区罩层必然与涂层高亮抢色。
  - 顺带收口（同一处的「第二份副本」问题）：
    - `theme.dom.test.ts` 里抄死的 10 个令牌期望值（`LIGHT_HIGHLIGHT_SECONDARY = '#fef9c3'` 等）全部删除 —— 它们正是本次变红的原因之一；改为 `expectInlined()`：**内联兜底必须与 theme.css 逐字一致**。
    - `theme.css` 里 `--tk-destructive` 的浅/深两个值对齐到「兜底里实际生效的值」（浅 `hsl(0 66% 48%)` / 深 `hsl(0 62% 55%)`）：内联样式优先级更高，所以**观感零变化**，只是不再让 CSS 与兜底各说各话。
  - 改后实测（脚本按 WCAG 算）：浅色激活 `#fef08a`+`#171717` = 15.4:1（饱和 0.98）、待切换 `#f3f4f6`+`#151519` = 16.5:1（饱和 0.14）；深色激活 15.4:1、待切换 `#44444b`+`#fafafa` = 9.3:1。
  - 用例（33 例，全量 1909 → 1913）：新增「激活项饱和 / 待切换项中性（靠饱和度区分）」2 条、「选区不自定义配色（两个编辑框都没有 `::selection` 规则）」1 条；「内联兜底一致性」取代原先只查键覆盖的版本（逐字比对全部主题相关令牌 + 「深色块令牌在浅色块都有基准值」「主题相关令牌不止个位数」两条前提断言）。
  - 变异验证：idle 改回 `#fef9c3`、把 `::selection` 规则加回去、兜底值漂移、兜底漏 `--tk-shadow-xs` —— 各自都让对应用例变红；复原后 33 例全绿。
  - 未闭环：仍需浏览器目测 —— ① `#f3f4f6` 在白卡片上**很淡**（与纯白约 1.1:1），若觉得候选几乎看不见，换成 `#e5e7eb`（gray-200）即可，只动一个令牌；② 深色抽屉里的同一组观感；③ 去掉自定义选区后，选中文字由浏览器默认配色绘制（选区文字色由 UA 提供），重点确认中文输入法组合期与跨段选中不出重影。⚠️ 调这一类样式**不能改字重/字号**：字宽一变，涂层与 textarea 就会错位重影。
  - 门禁 `format / lint / test(76 files / 1913) / build` 全绿（本机）。

- [x] **T29 智能解析的结果 tab 偏大（比同级胶囊胖一圈）**
  - 现象：用户截图反馈「结果 tab 太大」。
  - 取证（对截图逐像素量）：截图整体约 1.5× 缩放；深色激活 tab 竖直占 39 image px ≈ **26 CSS px**，而同屏「支持格式」的常规 chip 只有 22px —— 结果 tab 自己写死了更大的尺寸，`padding` 也宽 2px。
  - 修法（顺带把尺寸收进令牌）：新增 `--tk-control-h-xs: calc(22px * var(--tk-font-scale))`（行内小胶囊档），`.tw-detect__tab` 由 `height: 26px; padding: 0 10px` 改为 `height: var(--tk-control-h-xs); padding: 0 8px`，`.tw-detect__format-chip` 的写死 `22px` 也换成同一令牌。
  - 收益：① tab 与同级 chip 同高（视觉一致，用户要的「缩小一点」）；② 两个胶囊此前是写死 px，**不跟随字体缩放** —— 缩放调大时文字会顶出胶囊，现在一起缩放。
  - 用例（34 例，全量 1913 → 1914）：新增「行内胶囊尺寸一致」1 条 —— 两个选择器都必须走 `--tk-control-h-xs`（尺寸类样式没有排版引擎可断言，守的是「同一种视觉元素只认一个令牌」这条不变式）。
  - 变异验证：把结果 tab 的高度改回写死的 `26px` → 对应用例变红；复原后 34 例全绿。
  - 未闭环：需浏览器目测 22px 是否合适；若还要更矮，只改 `--tk-control-h-xs` 一个令牌即可（「支持格式」胶囊会一起变，这正是本轮想要的一致性）。
  - 门禁 `format / lint / test(76 files / 1914) / build` 全绿（本机）。

- [x] **T30 工程：把「省 token」落成机制（脚本静音 + 触发式交付 + 上下文纪律）**
  - 描述：T26 只做了验证分级（建议层），实测仍有大头开销：一次全量门禁原始输出 ~500 行 / 32 KB ≈ 8k tokens（真正浪费的是输出，不是时间）。
  - 修法：① 脚本静音 —— `format` 加 `--log-level warn`（165→5 行）、`test` 用 `--reporter=dot`（267→~170 行）、三处 vite 加 `--logLevel warn`、新增 `pnpm gate`；② §7.2 改成**用户触发**（默认不跑全量，纯视觉改动走 `pnpm test theme && pnpm build`）；③ 新增 §9 上下文与输出纪律 5 条 + 本文件条目预算。
  - 结果：门禁输出 **32 KB → 22 KB**（脚本静音，实测 223 行）；按 §7.1 前缀 `NODE_OPTIONS=--no-warnings` 后 **37 行 / 5.2 KB**（≈1.3k tokens，省掉 ~85%）。`AGENTS.md` 与 `TASKS.md` 的阅读成本有了明文预算。环境噪音（本机代理变量导致 vitest 76 个 worker 各喷 2 行 undici 警告）只能靠前缀，不进脚本（Windows 不兼容内联 env）。
  - 门禁 `pnpm gate` 全绿（76 files / 1914 例）。
