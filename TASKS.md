...
- **智能解析高亮**（T28 定稿）：① 浅色下待切换项 `#f3f4f6` 在白卡片上很淡（约 1.1:1），若几乎看不见就换 `#e5e7eb`（只动一个令牌）；② 深色抽屉里「激活 `#fef08a` + 深色字」与「待切换 `hsl(240 5% 28%)` + 近白字」是否醒目；③ 选区已改回浏览器默认配色 —— 重点看中文输入法组合期与跨段选中不出重影。⚠️ 调这类样式**不能改字重/字号**：字宽一变，涂层与 textarea 就会错位重影。
- **结果 tab 与「支持格式」胶囊**（T29）：两者现共用 `--tk-control-h-xs`（22px）；把字体缩放调到 1.5× 复看是否仍合适，要更矮只改这一个令牌。
- **划选解析面板**：输入框上限 220px 是否合适；结果较长被压缩后的观感（可能降到 ~200，极端到 96 下限）；极矮视口仍会出现最外层滚动条（有意的兜底）。
- **工具 tab 条**：首个 tab 文字与面板标题左缘是否对齐；默认宽度下 9 个 tab 不再横向溢出。
