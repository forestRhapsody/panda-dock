...
- **Cookie 分存储**：普通环境与无痕环境是两个隔离的 Cookie 存储（`storeId` `"0"` / `"1"`）。`chrome.cookies.*` 不传 `storeId` 时只用**后台自身所在的存储**（spanning 模式下恒为普通环境），所以无痕窗口会读到普通环境的 Cookie。所有 cookie 读写一律经 `resolveCookieTarget()` 按目标标签页反查 `storeId`（`getAllCookieStores()` 的 `tabIds`；扩展页没有 `sender.tab`，取 `lastFocusedWindow` 的活动标签页）。domain 的前导点只作展示（Chrome API / DevTools 都带），**作用域判据永远是 `hostOnly`**，任何地方都不要拿「有没有点」判断。
